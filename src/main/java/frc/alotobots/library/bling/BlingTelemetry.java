...
public class BlingTelemetry {
  ...
  private GenericEntry initializeShuffleboard() {
    blingTab
        .addBoolean("Bling Enabled", () -> BlingSubsystemConstants.BLING_ENABLED)
        .withPosition(0, 0);

    return blingTab.add("Current Color", "None").withPosition(0, 1).withSize(2, 1).getEntry();
  }
  ...
  private GenericEntry initializeAnimationEntry() {
    return blingTab.add("Current Animation", "None").withPosition(0, 2).withSize(2, 1).getEntry();
  }
  ...
  public void updateShuffleboard(Color currentColor, Animation currentAnimation) {
    ...
    currentAnimationEntry.setString(
        currentAnimation != null ? currentAnimation.getClass().getSimpleName() : "None");
  }
