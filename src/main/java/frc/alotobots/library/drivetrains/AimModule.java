package frc.alotobots.library.drivetrains;
...
public class AimModule {
  /**
   * Calculate the launch point given robot velocity, target coordinates, robot position, and
   * rotation.
   *
   ...
   * @param robotPose Robot X Position (Forward/Backwards facing speaker), Robot Y Position
   *     (Left/Right facing speaker), Robot Rotation in degrees (counterclockwise)
   * @return The launch point (Pose2d)
   */
  public static Pose2d calculateLaunchPoint(
      double xRobotVel, double yRobotVel, Pose2d targetPose, Pose2d robotPose) {
    // Convert rotation angle to radians
    double radians = targetPose.getRotation().minus(robotPose.getRotation()).getRadians();
    // Translate target coordinates to the origin relative to the robot position, so we can rotate
    // around the origin
    ...
    return new Pose2d(
        (targetX + (robotPose.getY() - targetY) / m), robotPose.getY(), robotPose.getRotation());
  }

  public static Rotation2d calculateRobotHeadingAlignShooterToPose(
      Pose2d targetPose, Pose2d robotPose) {
    double offset =
