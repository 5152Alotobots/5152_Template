...
public class SwerveDriveTelemetry {
  ...
  private void initializeOtherWidgets(SwerveDriveSubsystem swerveDrive) {
    driveTab
        .addBoolean("Tuning Mode", () -> Constants.Robot.TUNE_MODE)
        .withPosition(0, 3)
        .withSize(2, 1);
    driveTab.addBoolean("Flip Path", swerveDrive::getFlipPath).withPosition(0, 4).withSize(2, 1);
  }
  ...
  private ShuffleboardLayout initializeTunableParameters(SwerveDriveSubsystem swerveDrive) {
    ShuffleboardLayout tunableParamsList =
        driveTab
    ...
    maxAngularSpeedWidget =
        tunableParamsList.add("Max Angular Speed (rad/s)", swerveDrive.getMaxAngularSpeed());
  ...
  private void updateTunableParameters(SwerveDriveSubsystem swerveDrive) {
    ...
    double newMaxAngularSpeed =
        maxAngularSpeedWidget.getEntry().getDouble(swerveDrive.getMaxAngularSpeed());

    if (newMaxSpeed != swerveDrive.getMaxSpeed()
        || newMaxAngularSpeed != swerveDrive.getMaxAngularSpeed()) {
    ...
    if (newDriveKP != swerveDrive.getDriveKP()
        || newDriveKI != swerveDrive.getDriveKI()
        || newDriveKD != swerveDrive.getDriveKD()) {
    ...
    if (newTurnKP != swerveDrive.getTurnKP()
        || newTurnKI != swerveDrive.getTurnKI()
        || newTurnKD != swerveDrive.getTurnKD()) {
