...
public class LimelightTelemetry {
    ...
    private ShuffleboardLayout initializeShuffleboard(LimelightSubsystem limelightSubsystem) {
        ShuffleboardLayout detectedObjectsLayout = limelightTab
                .getLayout("Detected Objects", BuiltInLayouts.kGrid)
                .withProperties(Map.of("Number of columns", 1, "Number of rows", 3));

        detectedObjectsLayout.addStringArray("Highest Confidence", () -> limelightSubsystem.getDetectedObjectList().sortByConfidence().toArray());

        if (limelightSubsystem.getSubSysDrive() != null) {
            detectedObjectsLayout.addStringArray("Closest To Robot", () -> limelightSubsystem.getDetectedObjectList().sortByPose(limelightSubsystem.getSubSysDrive().getState().Pose).toArray());
        } else {
        ...
        limelightTab.addBoolean("Object Detected", () -> LimelightLib.getTV(LimelightSubsystemConstants.NN_LIMELIGHT));
    ...
    public void updateShuffleboard(LimelightSubsystem limelightSubsystem) {
        // The widgets are updated automatically through the lambda functions provided during initialization
