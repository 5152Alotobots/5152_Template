...
public class PathfindToBestObject extends InstantCommand {
  ...
  public void initialize() {
    ...
    Distance offset = Math.abs(Math.cos(angle)) > Math.abs(Math.sin(angle)) 
    ...
    Pose2d targetPose = new Pose2d(
        objectPose.getTranslation().plus(offsetTranslation),
        objectPose.getRotation()
    );
    
    Command command = pathPlanner.getPathFinderCommand(
        targetPose, 
