...
import frc.robot.crescendo.commands.expirimental.Cmd_AutoAngleAlignShootImmediately;
...
import frc.robot.crescendo.subsystems.shooter.commands.expirimental.Cmd_SubSys_Shooter_ShootImmediately;
    ...
    // Switch Robots
       switch (ROBOT) {
        ...
        default:
            ...
            NamedCommands.registerCommand("PickupNote", new Cmd_PickUpNote(intakeSubSys, shooterSubSys));
            NamedCommands.registerCommand("PickupNoteWithLimelight", new Cmd_PickUpNoteWithLimelight(intakeSubSys, shooterSubSys, drivetrain, limelightSubSys));
            NamedCommands.registerCommand("TransferNoteToShooter", new Cmd_TransferNoteToShooter(intakeSubSys, shooterSubSys));
            NamedCommands.registerCommand("AutoAngleAlignAndShoot", new Cmd_AutoAngleAlignShootImmediately(shooterSubSys, drivetrain));
            NamedCommands.registerCommand("EnableShooter", new InstantCommand(() -> shooterSubSys.setShooterOutput(ShooterDirection.OUT)));
            NamedCommands.registerCommand("DisableShooter", new InstantCommand(() -> shooterSubSys.setShooterOutput(ShooterDirection.OFF)));
            NamedCommands.registerCommand("ShootImmediately", new Cmd_SubSys_Shooter_ShootImmediately(shooterSubSys));
...
//        hmiStation.sliderIn.onTrue(new InstantCommand(sliderSubSys::sliderRetractCmd));
      ...
      hmiStation.shooterSpeakerPos.whileTrue(
              new Cmd_SubSys_Shooter_RotateToDegree(shooterSubSys, () -> ARM_PRESET_SPEAKER).withTimeout(2)
                      .andThen(new Cmd_SubSys_Shooter_SpinUpThenShoot(shooterSubSys, hmiStation.shooterShoot, hmiStation::shooterArmAxis))
      ).or(
              hmiStation.shooterAmpPos.whileTrue(
                      new Cmd_SubSys_Shooter_RotateToDegree(shooterSubSys, () -> ARM_PRESET_AMP).withTimeout(2)
                              .andThen(new Cmd_SubSys_Shooter_SpinUpThenShootAmp(shooterSubSys, hmiStation.shooterShoot, hmiStation::shooterArmAxis))
              )
      ).or(
